...
class City():
    ...
    Represents a city.
    """
    id_to_cities = dict() #associates an id to an instance of City
    name_to_cities = dict() # associates city names to a list of instances of City.
    ...
    def __init__(self, name: str, coordinates: Tuple[float, float], city_type: str,\
        ...
        # Associating the instance of City with its city_id 
        City.id_to_cities[self.city_id] = self
        
        # Check if self.name exists in the dictionary
        # Append it to the existing list on instances if self.name is in the dictionary
        if self.name in City.name_to_cities:
            City.name_to_cities[self.name].append(self)
        else:
            # If not, create a new list with the instance and associate it with self.name
    ...
    def __str__(self) -> str:
        ...
        """
        return '{} ({})'.format(self.name, self.city_id)
    ...
    def get_table_data(self) -> list[str]:
        ...
        """
        return [self.name, '{}'.format(self.coordinates), self.city_type, '{}'.format(self.population), '{}'.format(self.city_id)]
...
def get_cities_by_name(city_name: str) -> list[City]:
    ...
    """
    # return the list of cities by the same name stored in the name_to_cities dict
